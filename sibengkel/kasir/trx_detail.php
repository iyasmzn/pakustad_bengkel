...
if($_GET) {
	$kode = $_GET['code'];
	$sql = "SELECT trx.*, tmp_trx.*, barangjasa.*, pelanggan.*, karyawan.* FROM trx, tmp_trx, barangjasa, pelanggan, karyawan 
			WHERE trx.id_trx=tmp_trx.id_trx AND trx.id_kon=pelanggan.id_pelanggan AND tmp_trx.id_brg=barangjasa.id_brg
			AND trx.id_kasir = karyawan.id_karyawan AND trx.id_trx='". $_GET['code'] ."'";
